...
struct IntegrationsPrefsTab: View {
    ...
    @State private var confirmDeleteCredential: RegistryCredential?
    ...
    var body: some View {
                ...
                if credentials.isEmpty {
                    ...
                    ForEach(credentials) { cred in
                        HStack {
                            ...
                            Button("Delete") { confirmDeleteCredential = cred }
        ...
        .confirmationDialog(
            confirmDeleteCredential.map { "Delete credentials for \"\($0.registry)\"?" } ?? "Delete credentials?",
            isPresented: Binding(
                get: { confirmDeleteCredential != nil },
                set: { if !$0 { confirmDeleteCredential = nil } }
            ),
            titleVisibility: .visible
        ) {
            if let cred = confirmDeleteCredential {
                Button("Delete", role: .destructive) {
                    deleteCredential(cred)
                    confirmDeleteCredential = nil
                }
                Button("Cancel", role: .cancel) { confirmDeleteCredential = nil }
            }
        } message: {
            Text("This removes the stored username and Keychain password for this registry.")
        }
    }

    private func loadCredentials() {
        credentials = AppDatabase.shared.readOrDefault(.registryCredentials, default: [RegistryCredential]())
    }
...
struct RegistryCredentialSheet: View {
    ...
    private enum RegistryHost: Hashable {
        case ghcr, dockerHub, custom
    }

    @State private var hostOption: RegistryHost = .ghcr
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Form {
                Section("Registry") {
                    LabeledContent("Host") {
                        Picker("", selection: $hostOption) {
                            Text("GitHub Container Registry (ghcr.io)").tag(RegistryHost.ghcr)
                            Text("Docker Hub (docker.io)").tag(RegistryHost.dockerHub)
                            Text("Other…").tag(RegistryHost.custom)
                        }
                        .labelsHidden()
                    }
        ...
        .onAppear {
            if let c = credential {
                username = c.username
                switch c.registry {
                case "ghcr.io":   hostOption = .ghcr
                case "docker.io": hostOption = .dockerHub
                default:
...
extension URL {
    var abbreviatingWithTilde: String {
        path.replacing(FileManager.default.homeDirectoryForCurrentUser.path, with: "~")
    }
}
